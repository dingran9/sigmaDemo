...
<header id="header" class="custom-header" >
    <div id="logo-group" class="nominify-logo-group">
        ...
        <span id="logo" style="margin-top: 0px;"> <img id="logo-img" class="img img-responsive" src="../../static/smartadmin/img/logo_biology.png" alt="SmartAdmin"> </span>
...
<script src="../../static/js/biology/common.js?version=4.2.6.2"></script>
...
<script>
    ...
    var datas = <%=session.getAttribute("idcManagerPermissions")%>;
    for(var i=0;i<datas.length;i++){
        if(datas[i].name=="userOperateLog"){
            $("li[name='per_operateLog']").show().addClass("showLi");
        }else if(datas[i].name=="managerOperateLog"){
            $("li[name='per_operateLog']").show().addClass("showLi");
        }
        $("li[name='per_"+datas[i].name+"']").show().addClass("showLi");
        //$("li[name='per_"+datas[i].name+"']").show().addClass("showLi");
        $("li[name='per_"+datas[i].name+"']").closest("li[name^='pers_']").show().addClass("showLi");
    }
    //TODO 后期优化 需要修改 permission 结构
    /*if($("li[name='pers_system'] ul li[style='display: none;']").length==7){
        $("li[name='pers_system']").hide();
    }
    if($("li[name='pers_products'] ul li[style='display: none;']").length==3){
        $("li[name='pers_products']").hide();
    }
    if($("li[name='pers_console'] ul li[style='display: none;']").length==7){
        $("li[name='pers_console']").hide();
    }
    if($("li[name='pers_billcoupon'] ul li[style='display: none;']").length==2){
        $("li[name='pers_billcoupon']").hide();
    }*/
    $("#manager-is-staff").html("<%=session.getAttribute("idcManagerIsStaff")%>");
    $("#manager-is-da").html("<%=session.getAttribute("idcManagerIsDA")%>");
    $("#managerId").html("<%=session.getAttribute("userAccountId")%>");
