...
var dragEffect = {
	...
	zfBounce: {//指数衰减的反弹缓动。
		easeIn: function(t,b,c,d){
			return c - dragEffect.zfBounce.easeOut(d-t, 0, c, d) + b;
		},
		easeOut: function(t,b,c,d){
			if ((t/=d) < (1/2.75)) {
		...
		easeInOut: function(t,b,c,d){
			if (t < d/2) return dragEffect.zfBounce.easeIn(t*2, 0, c, d) * .5 + b;
			else return dragEffect.zfBounce.easeOut(t*2-d, 0, c, d) * .5 + c*.5 + b;
		}
	}
}
////////////////////以上都是算法
...
function animate(ele,obj,nDuration,effectType,fnCallback){
	...
	//effectType 1,2,3,4
	if(!effectType){
		var fn=dragEffect.zfLinear;//直线，匀速的 1
		...
		switch(effectType){
			case 1:
				...
				break;
			case 2:
				var fn=dragEffect.Quart.easeInOut;//直线，匀速的 1
				break;
			case 3:
				var fn=dragEffect.Elastic.easeOut;//直线，匀速的 1
				break;
			case 4:
				var fn=dragEffect.zfBounce.easeOut;//直线，匀速的 1
				break;
			default:
		...
		var fn=dragEffect.zfLinear;	
		}else{			
			var fn=dragEffect[effectType[0]][effectType[1]];		
		}		
