/**
 * Created by Zhangly on 2014/10/10.
 */
...
$(document).ready(function() {
    ...
    $('.minifyme').click(function(e) {
        ...
        if($('body').hasClass("minified")){
            $("#logo-img").attr("src","../../static/images/biologylogo.png").addClass("minify-logo-img");
            $("#logo-group").addClass("minify-logo-group");
        }else{
            $("#logo-img").attr("src","../../static/smartadmin/img/logo_biology.png").removeClass("minify-logo-img");
            $("#logo-group").removeClass("minify-logo-group");
        }
        e.preventDefault();
    });
    ...
    // LOGOUT BUTTON
    $('#logout #editPasswordBtn').click(function(e) {
        clearForm("edit_password");
        $("#edit_password_modal").find("em").remove();
        $("#edit_password_modal").find("hsa-error").remove();
        $("#edit_password_modal").modal('show');

        e.preventDefault();
    });
    $('#logout a:last').click(function(e) {
        //get the link
        var $this = $(this);
        ...
        // ask verification
        $.SmartMessageBox({
            title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> 退出 <span class='txt-color-orangeDark'><strong>" + $('#logout').find('span:first').text() + "</strong></span> ?",
            content : $.logoutMSG || "确定是否要退出当前用户？",
            buttons : '[取消][确定]'
...
function checkURL() {
    ...
    if (url) {
        ...
        if($('nav li:has(a[href="#' + url + '"])').attr("style")=="display: none;"){
            var $this = $('nav > ul > li[style!="display: none;"] > a');//> a[href!="#"]
            if($this.attr('href')=="#"){
                $this = $('nav > ul > li[style!="display: none;"] > ul > li[style!="display: none;"] > a');
            }
      ...
      //  document.title = (title || document.title);
        ...
        var $this = $('nav > ul li.showLi:first-child > a[href!="#"]');


        //update hash
        window.location.hash = $this.attr('href');
        if(window.location.hash==""|| window.location.hash=='undefined'){
            window.location.hash='#jsp/dict/dictmanager.html';
        }
    }
...
function drawBreadCrumb() {
    ...
    $.bread_crumb.empty();
    $.bread_crumb.append($("<li style='font-size: 16px;margin-left:14px;color: #666666;'>管理中心</li>"));
...
function initProductRegion(obj,callback,regionId){
    var region = document.getElementById(obj).options;
    clearOptions(region);
    addOption(region,"---正在加载中---","");
        ...
        if(objs.httpCode=="200"){
            ...
            if (objs.datas.length > 0) {
                for (var i = 0; i < objs.datas.length; i++) {
                    addOption(region, objs.datas[i].name, objs.datas[i].id);
                }
            }
...
function initProductImage(obj,callback){
    var image = document.getElementById(obj).options;
    clearOptions(image);
    ...
    doPost("/action/systemImage/notUseList",{regionId:$("#product_region").val()},function(objs){
        if(objs.httpCode=="200"){
            ...
            if (objs.datas.length > 0) {
                ...
                addOption(image, "该地域下已无可添加的镜像产品", "");
            }
            callback();
        }else{
            console.log("code :" + objs.code + "  msg:" + objs.message);
 ...
 * 地域列表
 */
function listRegionsBtns(id,funcId,callback){
    doPost("/action/region/findRegionList",{},function(objs){
        if(objs.httpCode == "200") {
            ...
            for(var i=0;i<data.length;i++){
                if(i==0){
                    str +=" <a data-value='"+data[i].id+"' onclick='"+funcId+"(this)' id ='"+id+","+data[i].regionUuid+"' style='width:120px;margin:13px'  class='bs-btn btn-primary'>"+data[i].name+"</a>";
                }else{
                    str +=" <a data-value='"+data[i].id+"' onclick='"+funcId+"(this)' id ='"+id+","+data[i].regionUuid+"' style='width:120px;margin:13px'  class='bs-btn '>"+data[i].name+"</a>";
                }
            }
            $("#"+id).empty().append(str);
            callback();
